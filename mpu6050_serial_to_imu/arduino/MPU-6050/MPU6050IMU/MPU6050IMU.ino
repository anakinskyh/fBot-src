...
//  quaternion
  memcpy(teapotPacket+2,&q[0],sizeof q[0]);
  memcpy(teapotPacket+6,&q[1],sizeof q[1]);
  memcpy(teapotPacket+10,&q[2],sizeof q[2]);
  memcpy(teapotPacket+14,&q[3],sizeof q[3]);
//  gyro
  memcpy(teapotPacket+18,&gx,sizeof gx);
  memcpy(teapotPacket+22,&gy,sizeof gy);
  memcpy(teapotPacket+26,&gz,sizeof gz);
//accelerometer
  memcpy(teapotPacket+30,&gx,sizeof gx);
  memcpy(teapotPacket+34,&gy,sizeof gy);
  memcpy(teapotPacket+38,&gz,sizeof gz);

//  temperature
  memcpy(teapotPacket+42,&temperature,sizeof temperature);
...
// 50 mag
  ...
  memcpy(teapotPacket+48,&event.magnetic.x,sizeof event.magnetic.x);
  memcpy(teapotPacket+52,&event.magnetic.y,sizeof event.magnetic.y);
  memcpy(teapotPacket+56,&event.magnetic.z,sizeof event.magnetic.z);
  
  
  Serial.write(teapotPacket,62);
